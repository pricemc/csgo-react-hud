...
export default class Bomb extends React.Component<IProps> {
  ...
  getCaptionGif = (type: "defusing" | "planting", player: Player | null) => {
    if(!player) return null;
    if(type === "defusing"){
      return <>
        <I.Defuse height={22} width={22} fill="var(--color-new-ct)" />
  ...
  render() {
    ...
    return (
      <div className={`defuse_plant_gif ${side} ${timer && timer.active ? 'show' :'hide'}`}>
        {
          timer ?
          <div className={`defuse_plant_caption`}>
            {this.getCaptionGif(timer.type, timer.player)}
          </div> : null
        }
